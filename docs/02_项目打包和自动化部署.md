...
伴随着DevOps一起出现的两个词就是**持续集成**和**持续交付(部署)**：
...
3.配置网络安全组：什么协议可以访问，哪些端口开放，允许哪些IP地址访问。
...
* rpm：全称为**The RPM Package Manage**，是Linux的一个软件包管理器；
...
systemctl enable jenkins # 随着服务器启动而启动
```
...
通过服务器的ip和端口，访问jenkins的图形化界面，根据提示，去对应目录下查看管理员密码（cat xxx/xxx/xx），安装推荐的插件。
...
nginx可以做请求的代理，可配置负载均衡，高可用，高并发，支持很多其它协议，如https，rtsp/rtmp（流媒体协议）
